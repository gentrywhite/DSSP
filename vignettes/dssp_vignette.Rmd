...
meuse.fit<-DSSP(N,X.train,Y.train,function(x) -2*log(1+x), pars=c(0.001,0.001))
...
```{r, eval=TRUE,include=TRUE, out.width='50%', fig.height=2.5, fig.width=3.5, fig.show='hold',fig.align='center'}
...
#grid.arrange(eta.densityplot,delta.densityplot,ncol = 2)

```

```{r, eval=TRUE,echo=FALSE,include=TRUE,fig.show='hold'}
...
```{r,eval=TRUE,include=TRUE, fig.show='hold'}
