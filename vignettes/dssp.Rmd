...
meuse.fit<-DSSP(formula=log(zinc)~1, data=meuse.train, N=N,function(x) -2*log(1+x), fitted.values = TRUE,pars=c(0.001,0.001))
...
```{r,eval=TRUE,include=TRUE}
Y.pred<-DSSP.predict(meuse.fit,meuse.valid)
Y.pred<-exp(Y.pred)
